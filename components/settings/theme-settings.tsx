...
export function ThemeSettings() {
  ...
  return (
    <Card>
      <CardHeader>
        <CardTitle className="text-sm">{t("theme.title")}</CardTitle>
      ...
      <CardContent className="grid gap-2 sm:grid-cols-2">
        {THEME_PROFILES.map((item) => {
          ...
          return (
            ...
            >
              <span aria-hidden className="size-5 shrink-0 rounded-full border border-current/20" style={{ background: item.swatch }} />
              <span className="min-w-0 flex-1">
                <span className="flex items-center gap-2 font-medium">
                  {item.label}
                  {active && <CheckIcon className="size-3.5" />}
                </span>
                <span className="mt-1 block whitespace-normal text-xs font-normal opacity-70">{t(`theme.${item.value}`)}</span>
              </span>
            </Button>
          )
        })}
