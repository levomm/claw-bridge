...
class AppUpdater(
    ...
    fun download(info: AppUpdateInfo, progress: (Long, Long) -> Unit): File {
        ...
        val partial = File(directory, "claw-bridge-${BuildConfig.APP_VARIANT}.apk.part")
        val target = File(directory, "claw-bridge-${BuildConfig.APP_VARIANT}.apk")
    ...
    private fun verifyApk(apk: File, expectedVersionCode: Long) {
        ...
        check(archive.packageName == context.packageName) { "Update package name does not match CLAW Bridge" }
